...
public class node_structure_handler : MonoBehaviour
{
    ...
    // written and comments by chatGPT start
    // func to get list of links      >>>>>>>>>>>>>>>>>>
    public List<string> get_links(string url)
    {
        ...
        foreach (Match match in matches)
        {
            ...
            {
                string absoluteUrl = new Uri(new Uri(url), href).AbsoluteUri;
    ...
    // func to get list of links      >>>>>>>>>>>>>>>>>>
    // written and comments by chatGPT end
    ...
    public void expand_node()
    {
        ...
        if (!expanded)
        {
            ...
            // here we use the chatGPT function to get all the links connected to THIS node
            // keep reading to understand where node_url is coming from, OR ctrl + F "]][["
            List<string> connected_urls = get_links(node_url);
